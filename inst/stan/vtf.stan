data {
  int n;
  int n_id; // total ids across all subs
  int n_sub;
  int sub_by_vox[n_id]; // locates the id's sub
  int n_unique_orientations;
  vector<lower = -pi(), upper = pi()>[n_unique_orientations] unique_orientations;
  int n_unique_orientations_vox[n_id];
  int<lower = 0, upper = n_unique_orientations> ori_by_vox[n_id, n_unique_orientations];
  vector[n] y;
  int X[n];
...
parameters {
  ...
  matrix[n_id, 2] meanAngleVector;

  // --- gamma multiplier --
  real<lower = 0> s_gamma_loc;
  real<lower = 0> s_gamma_scale;
  vector<lower = 0>[n_id] v_gamma;

  // -- concentration --
  real<lower = 0> v_kappa_loc;
  real<lower = 0> v_kappa_scale;
  vector<lower = (-v_kappa_loc)/v_kappa_scale>[n_id] v_kappa_raw;
  ...
  vector<lower=ntfp_min>[n_id] v_ntfp;
}
transformed parameters{
  vector<lower=0>[n_id] v_kappa = fma(v_kappa_raw, v_kappa_scale, v_kappa_loc);
  vector<lower=0>[n_id] lengthOfMeanAngleVector = sqrt(rows_dot_self(meanAngleVector));
  matrix[n_id, 2] meanAngleUnitVector = append_col(meanAngleVector[,1] ./ lengthOfMeanAngleVector,
  meanAngleVector[,2] ./ lengthOfMeanAngleVector);
  vector<lower = -pi(), upper = pi()>[n_id] meanAngle;

  for(v in 1:n_id) meanAngle[v] = atan2(meanAngleUnitVector[v,2], meanAngleUnitVector[v,1]);
}
model{
  ...
  target += -normal_lccdf(0 | sigma_loc, sigma_scale) * n_id;
  ...
  target += -normal_lccdf(0 | s_gamma_loc, s_gamma_scale) * n_id;
  ...
  target += -normal_lccdf((-v_kappa_loc)/v_kappa_scale | 0, 1) * n_id;
  ...
  target += -normal_lccdf(ntfp_min | s_ntfp_loc, s_ntfp_scale) * n_id;

  {
    // unless all ids were tested with the same number of orientations,
