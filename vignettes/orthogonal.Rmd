...
Ideally, we could assess changes in the voxel tuning function by simply plotting them. That is an easy enough plot to generate. Here are the tuning functions for 6 voxels.
...
Unfortunately, most voxels have weak tuning (i.e., across orientations, the lines are nearly flat). Moreover, the activity at a given orientation is quite variable, and so it's challenging to distinguish an additive shift from a subtle multiplicative gain within a single voxel.
